...
import { calculateCreditStatus, formatInputDate } from '../lib/creditCalculations';
...
export const exportClientsToExcel = (users: User[]) => {
  ...
  const rows = clients.map((u, idx) => {
    ...
    return {
      ...
      'Estado Calculado': statusInfo.statusLabel,
      'Fecha Otorgamiento': u.loanStartDate || u.createdAt || formatInputDate(new Date()),
      'Fecha Vencimiento': statusInfo.dueDate,
      'Modalidad de Pago': statusInfo.frequency === 'quincenal' ? 'Quincenal (15 días)' : 'Mensual (30 días)',
      'Pagos Calculados': statusInfo.frequencyLabel,
      'Saldo Disponible (COP)': u.balance,
      'Línea de Crédito (COP)': u.creditLimit,
      'Deuda Actual (COP)': u.creditUsed,
      'Cuota Calculada (COP)': statusInfo.installmentAmount,
      'Plazo Total (Días)': u.paymentTermDays ?? 30,
      'Tasa Interés Diario (%)': u.dailyInterestRate ?? 0.05,
  ...
  worksheet['!cols'] = [
    ...
    { wch: 18 }, // Due Date
    { wch: 22 }, // Frequency
    { wch: 24 }, // Calculated Installments
    { wch: 20 }, // Balance
    { wch: 22 }, // Credit Limit
    { wch: 18 }, // Credit Used
    { wch: 18 }, // Quota
  ...
  const activeCount = clients.filter((c) => calculateCreditStatus(c).computedStatus === 'active').length;
  const pendingCount = clients.filter((c) => calculateCreditStatus(c).computedStatus === 'pending').length;
  const blockedCount = clients.filter((c) => calculateCreditStatus(c).computedStatus === 'blocked').length;

  const summaryData = [
    { 'Concepto': 'Fecha de Exportación', 'Valor': new Date().toLocaleString('es-CO') },
    { 'Concepto': 'Total de Clientes', 'Valor': clients.length },
    { 'Concepto': 'Clientes Activos (Al Día)', 'Valor': activeCount },
    { 'Concepto': 'Clientes en Mora', 'Valor': pendingCount },
    { 'Concepto': 'Clientes Cancelados ($0)', 'Valor': blockedCount },
...
export const downloadBulkUploadTemplate = () => {
  ...
  const instructions = [
    ...
    { 'Columna': 'Cupo_Credito', 'Obligatorio': 'Opcional', 'Descripción': 'Línea de crédito aprobada en COP (máx. $5.000.000 COP, ej. 2000000)' },
    ...
    { 'Columna': 'Cuota_Mensual', 'Obligatorio': 'Opcional', 'Descripción': 'Valor de la cuota en COP (por defecto 1250000)' },
    { 'Columna': 'Cuotas_Totales', 'Obligatorio': 'Opcional', 'Descripción': 'Total de cuotas del crédito (por defecto 12)' },
    { 'Columna': 'Plazo_Dias', 'Obligatorio': 'Opcional', 'Descripción': 'Plazo del ciclo en días: 15 (Quincenal) o 30 (Al mes)' },
...
export const parseBulkClientsFile = async (file: File): Promise<ParsedClientRow[]> => {
  ...
  jsonRows.forEach((row, index) => {
    ...
    const parsedCreditLimit = Math.max(
      0,
      Number(String(normalized['cupocredito'] || normalized['cupo'] || normalized['limite'] || 1000000).replace(/[^0-9.]/g, '')) || 1000000
    );
    // Limitar todo el sistema de créditos a máximo 5.000.000 COP
    const creditLimit = Math.min(5000000, parsedCreditLimit);
    ...
    const rawTerm = Number(String(normalized['plazodias'] || normalized['plazo'] || normalized['dias'] || 30).replace(/[^0-9]/g, '')) || 30;
    // Pagos de 15 días y al mes (30 días)
    const paymentTermDays = rawTerm === 15 ? 15 : 30;
